...
import { Button, Table, TableHeader, TableRow, TableCell, TableBody, TableHeaderCell, Title2, Text } from '@fluentui/react-components'
...
export default function UsersList() {
  ...
  const [page, setPage] = useState({ content: [], pageNumber: 0, pageSize: sizeParam, totalElements: 0, totalPages: 0 })
  ...
  return (
    <div>
      ...
      <div className="pager">
        <Button disabled={page.pageNumber<=0} onClick={() => onPageChange(page.pageNumber-1)}>Prev</Button>
        <Text>Page {page.pageNumber+1} / {page.totalPages}</Text>
        <Button disabled={page.pageNumber+1>=page.totalPages} onClick={() => onPageChange(page.pageNumber+1)}>Next</Button>
      </div>
    </div>
  )
}