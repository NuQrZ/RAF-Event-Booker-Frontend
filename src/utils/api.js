...
export async function api(path, { method = 'GET', headers = {}, body, auth = true } = {}) {
    ...
    if (res.status === 403) { window.location.href = '/no-access'; throw new Error('Forbidden') }
    const contentType = res.headers.get('content-type') || ''
    const isJson = contentType.includes('application/json')
    const data = isJson ? await res.json() : await res.text()
    if (!res.ok) { const msg = isJson && data && data.message ? data.message : res.statusText; throw new Error(msg || 'Request failed') }
    return { data, res }
}
